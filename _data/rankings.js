...
function parseCsvRow(rowString) {
    if (!rowString) return [];
    // This robust regex handles CSVs with quoted commas correctly.
    const values = rowString.match(/(".*?"|[^",]+)(?=\s*,|\s*$)/g) || [];
    // Clean up each value by removing surrounding quotes and trimming whitespace.
...
module.exports = async function() {
    ...
    const lastUpdated = players.length > 0 ? players[0]['Last Updated'] : "N/A";

    if (players.length === 0) {
        throw new Error("No valid player data was found in the sheet.");
    }
